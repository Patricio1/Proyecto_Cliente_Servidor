...
{
    class GestionarReporteDetalleFactura
    {
        OracleConnection connection;
        OracleDataAdapter adaptador;

        public GestionarReporteDetalleFactura()
        {
            adaptador = new OracleDataAdapter();
        }

        public Boolean buscarRegistros(int numeroFactura)
        {
            ...
            try
            {
                ...
                string sql = "SELECT* FROM INVOICES F INNER JOIN CUSTOMERS C ON F.CUSTOMERID= C.CUSTOMERID WHERE F.INVOICEID="+numeroFactura;
                string sql1 = "SELECT* FROM INVOICELINEITEMS D INNER JOIN PRODUCTS P ON D.PRODUCTCODE=P.PRODUCTCODE WHERE D.INVOICEID="+numeroFactura;
